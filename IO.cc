// Copyright (c) 2016 Sudarsun Kannan.
// All Rights Reserved.
//********************************/
// Developement under progress. Not a completed version
...
void* thrdrandwrite (void *arg) {
  ...
  while (bytes < filesize) {
    if(iswrite) { 
      size_t temp = 0; 
      //temp = pwrite(fd, buf, buffsize, offset);
      temp = write(fd, buf, buffsize);
      bytes += temp;
      fprintf(stderr,"bytes %zu temp %zu fd %d\n",bytes, temp, fd);
    }else {
...
int OpenFiles(char *fname, const char *mode, int intmode) {
...
int 
do_IO(int seq, char *fname, int buffsize, int isWrite, const char *mode, int intmode) {
  ...
  OpenFiles(fname, (const char *)mode, intmode);
  if(!g_filesize)
    g_filesize = FILESIZE;
  ...
  printf("%lf MB/sec, Elapsed %lf, bytes %ld\n",g_thrput, g_elapsed.count(), g_iobytes);
  for(i = 0; i < g_num_threads; i++) {
    fclose(g_fparr[i]);
  }
...
int main(int argc, char *argv[]) {
  ...
  //read_zipf_number();
   ...
   if(mode == 0)
     do_IO(isSeq, fname, buffsize, WRITE, (const char*)WRMODE, O_RDWR | O_DIRECT);
   else if(mode == 1)
     do_IO(isSeq, fname, buffsize, READ, (const char*)RDMODE, O_RDONLY | O_DIRECT);
   else if(mode == 2)
     do_IO(isSeq, fname, buffsize, APPEND, (const char*)APPENDMODE, O_APPEND);
